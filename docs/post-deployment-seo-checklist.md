...
- El redirect `www` → sin-`www` en `next.config.ts` **no tiene efecto hasta que el dominio real esté en DNS** (hoy el sitio corre en un dominio temporal de Vercel). No hay riesgo de bucle porque la condición `has: [{ type: "host", value: "www.<domain>" }]` solo aplica al host exacto.
