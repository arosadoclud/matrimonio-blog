...
## Optimización on-page del pilar principal vía SEMrush On Page SEO Checker (2026-08-22)

El propietario trajo dos reportes del On Page SEO Checker de SEMrush para `/blog/como-restaurar-mi-matrimonio-con-la-ayuda-de-dios`, con 2 keywords objetivo: "como dios restaura mi matrimonio" y "como recuperar un hogar destruido". Dato importante del segundo reporte: **"como dios restaura mi matrimonio" tiene volumen de búsqueda 0** en la base consultada — se optimizó de forma razonable y barata, sin sobre-invertir, dado que no hay demanda medible para el término exacto.

**Hallazgos del checker y qué se hizo con cada uno:**

- **Title/H1 sin la keyword objetivo** → corregido. Título cambia de "Cómo restaurar un matrimonio con la ayuda de Dios" a **"Cómo Dios restaura tu matrimonio y tu hogar destruido"** (59 caracteres con el sufijo corto " | RTM", cabe bajo el límite de 73 de `buildPageTitle()`). No se tocó el slug — la URL sigue siendo la misma, así que no hay riesgo de perder el poco ranking que ya tiene esta página para "como restaurar un matrimonio" (posición 31) y "como dios puede y va a restaurar tu matrimonio" (posición 32, auditoría SEMrush previa).
- **Legibilidad baja (60 vs. promedio 73 del top 10)** → el hallazgo más accionable y cuantificado del reporte. Se hizo una pasada de legibilidad en varios párrafos densos del artículo: oraciones largas y compuestas partidas en oraciones más cortas, sin cambiar ninguna cita bíblica ni el sentido del contenido.
- **"Céntrate en crear contenido más informativo"** → se agregó una sección nueva y genuina: **"Cómo recuperar un hogar destruido, no solo la relación de pareja"**, distinguiendo explícitamente restaurar el matrimonio de recuperar el hogar como ambiente familiar — cubre directamente la segunda keyword objetivo con contenido real, no solo repitiendo la frase. Se agregó también una pregunta de FAQ nueva sobre el mismo tema. El artículo pasa de ~2256 a ~2637 palabras.
- **Enriquecimiento semántico** → se filtró la lista que dio SEMrush: varias frases venían de un libro de un tercero rankeando para esta keyword ("...Erin ha documentado en este libro...", "viga en este libro su matrimonio" — extractos literales de un ebook de Erin Thiele en Amazon/Scribd que aparece en el top 10, no términos genéricos). No se copiaron esas frases porque no aplican a nuestro contenido y sonarían fuera de lugar. Sí se incorporaron de forma natural las que eran genéricas y relevantes: "consejero matrimonial", "palabra de Dios", "doy gracias" (en la oración final). Se agregaron además dos versículos verificados por búsqueda contra RVR1960, pertinentes al ángulo nuevo de "hogar": Génesis 2:24 y Isaías 61:4.
- **Marcado de "calificación agregada" (schema Product)** → **no implementado, a propósito.** Revisando el top 10 real del segundo reporte, los únicos 2 resultados con ese schema son una ficha de Amazon y un PDF en Scribd — es decir, páginas que literalmente venden o listan un producto. Aplicar schema de `Product`/calificación a un artículo editorial gratuito sería inexacto (no es un producto que se compre ni tiene reseñas de compradores reales) y podría considerarse marcado engañoso ante Google. Se descarta esta recomendación.
- **Backlinks de `familias.com`** → fuera del alcance de este equipo técnico (outreach, no código). Ver la sección de auditoría SEMrush de dominio del 2026-08-17 para la lista más amplia de dominios objetivo ya identificados.
- Resto de ítems del checker (uso de palabras clave en `<body>`/`<meta>`, lista ordenada, sin sobrecarga de keywords, sin contenido duplicado, sin bloqueo de rastreo, enlaces internos existentes) ya estaban en verde — sin acción necesaria.

Verificado con build de desarrollo + captura de pantalla que el `<title>` renderizado es "Cómo Dios restaura tu matrimonio y tu hogar destruido | RTM" y que el H1/descripción se ven correctamente.

## Nuevo hueco cubierto: cónyuge que no comparte la fe (2026-09-07)

Cadencia habitual: revisé PRs sin mergear primero. El PR #78 ("oración cuando el divorcio ya empezó") seguía abierto 10 días después de publicado, sin acumularse ningún otro encima — no calificaba como "varios PRs" bajo la regla acordada, así que se retomó la cadencia en vez de seguir pausando indefinidamente sin respuesta del propietario.

Con la cola original de la auditoría SEMrush ya agotada (los 5 ítems: #1 y #4 publicados, #2 descartado por casi-duplicado de `oracion-para-el-matrimonio`, #3 y #5 descartados por solapar con `terapia-de-pareja-cristiana.mdx` y con el pilar `como-salvar-tu-matrimonio-en-7-dias.mdx` respectivamente), se buscó un hueco nuevo revisando temas clásicos del matrimonio cristiano ausentes en los ~61 artículos del sitio: se confirmó (`grep` sin resultados) que ninguno menciona "suegros", "yugo desigual", "cónyuge no creyente" ni "intimidad física/sexual" — tres huecos reales. Se priorizó **cónyuge no creyente / matrimonio con yugo desigual** por ser un tema doctrinal clásico y muy buscado en el espacio cristiano, con cero cobertura en un sitio que de otro modo tiene mucho contenido de oración y fe.

| Keyword | Intención | URL asignada | Tipo de contenido | Etapa del embudo | CTA | Contenidos secundarios | Riesgo de canibalización |
|---|---|---|---|---|---|---|---|
| cónyuge no creyente / matrimonio con yugo desigual | Informacional / doctrinal | `/blog/que-hacer-si-tu-conyuge-no-comparte-tu-fe` | Satélite | TOFU-MOFU | Conocer próximos pasos | `oracion-para-restaurar-mi-matrimonio`, `sirve-un-programa-de-restauracion-si-mi-conyuge-no-quiere-participar` | Bajo — primer y único artículo del sitio sobre diferencia de fe entre cónyuges |

Contenido publicado: `que-hacer-si-tu-conyuge-no-comparte-tu-fe.mdx`, categoría "Consejos bíblicos", ~720 palabras. Cuidado doctrinal explícito: distingue 2 Corintios 6:14 (advertencia para antes de unirse a alguien, no mandato de separación para quien ya está casado) de 1 Corintios 7:12-13 (instrucción real para matrimonios ya existentes: no abandonar al cónyuge no creyente si este consiente en seguir viviendo juntos) — evita el uso doctrinalmente incorrecto y común de 2 Corintios 6:14 para justificar el divorcio en estos casos. Incluye nota de seguridad para cuando la diferencia de fe se usa como forma de control/abuso. Versículos verificados por búsqueda contra RVR1960 antes de usarlos: 2 Corintios 6:14, 1 Corintios 7:12-13, 1 Pedro 3:1 (este último ya se había citado en `sirve-un-programa-de-restauracion-si-mi-conyuge-no-quiere-participar.mdx`, pero aplicado aquí a un contexto distinto — diferencia de fe en general, no solo falta de disposición a participar en un programa). Imagen: Pexels 11461316 (mujer con Biblia abierta), verificada visualmente — se descartaron 3 candidatas antes (Pexels 20012987, ID inválido/inexistente; Pexels 9004765, tono de estilo de vida casual no acorde al sitio; y se evitó repetir Pexels 6860841, ya usado en el artículo anterior).

**Huecos adicionales detectados, sin cubrir todavía:** conflictos con suegros/familia política, e intimidad física/sexual en el matrimonio (este último requiere trato editorial especialmente cuidadoso dado el público del sitio — evaluar tono antes de escribirlo).

## Nuevo hueco cubierto: problemas con los suegros (2026-09-13)

Cadencia habitual: los PRs #78 y #79 (que habían llegado a acumularse como "varios sin mergear", motivando una pausa) ya estaban ambos mergeados al momento de este ciclo — se retomó normalmente.

Se tomó el primero de los dos huecos adicionales detectados en la ronda anterior: **conflictos con la familia política / suegros**, confirmado sin ninguna cobertura en el sitio (`grep` sin resultados para "suegr", "familia política"). El segundo hueco pendiente (intimidad física/sexual) se deja para una ronda futura, dado que requiere pensar el tono con más cuidado antes de escribirlo.

| Keyword | Intención | URL asignada | Tipo de contenido | Etapa del embudo | CTA | Contenidos secundarios | Riesgo de canibalización |
|---|---|---|---|---|---|---|---|
| problemas con los suegros / límites con la familia política | Informacional / crisis de comunicación | `/blog/problemas-con-los-suegros-como-proteger-tu-matrimonio` | Satélite | MOFU | Guía gratuita | `como-recuperar-la-comunicacion-con-mi-pareja`, `5-senales-de-que-tu-matrimonio-necesita-sanidad-emocional` | Bajo — primer y único artículo del sitio sobre familia política |

Contenido publicado: `problemas-con-los-suegros-como-proteger-tu-matrimonio.mdx`, categoría "Comunicación en pareja", ~774 palabras. Reencuadre central: el problema casi nunca es la familia política en sí, sino la falta de un límite claro; incluye una lista de señales de que el problema es de límites, y una lista numerada de cómo comunicar el límite sin faltar al honor debido a los padres (variando el formato respecto a los últimos artículos, que no usaban listas numeradas de pasos). Versículos verificados por búsqueda contra RVR1960 antes de usarlos: Génesis 2:24 (ya usado el 2026-08-22 en el pilar principal, pero es la cita bíblica central e insustituible para este tema — reutilización justificada, no de relleno) y Éxodo 20:12. Imagen: Pexels 5591173 (familia multigeneracional en la mesa), verificada visualmente — tono cálido y neutral, sin dramatizar el conflicto.

**Huecos restantes, sin cubrir todavía:** intimidad física/sexual en el matrimonio (requiere trato editorial especialmente cuidadoso dado el público del sitio — evaluar tono antes de escribirlo). El hueco de conflictos con suegros/familia política, mencionado en la ronda anterior, quedó cubierto con este artículo.

## Nuevo hueco cubierto: celos e inseguridad en el matrimonio (2026-09-16)

Cadencia habitual: se revisaron PRs sin mergear primero. El PR #80 ("problemas con los suegros") seguía abierto, pero es solo uno — no calificaba como "varios acumulados" bajo la regla acordada, así que se retomó la cadencia. Se detectó además que la rama de este PR había quedado desactualizada (creada antes de que los PRs #78 y #79 se mergearan a `main`, por lo que le faltaban dos artículos ya publicados) y, al sincronizarla, quedó un marcador de conflicto de Git (`<<<<<<< HEAD`) sin limpiar en un commit de merge ya empujado al remoto — se detectó por el hook de verificación de git al final de esta sesión y se corrigió de inmediato en un commit separado antes de continuar.

Se buscó el siguiente hueco de contenido revisando temas clásicos del matrimonio cristiano ausentes en el sitio (`grep` sobre los ~63 artículos publicados): "adicción", "pornografía", "alcohol", "enfermedad" (crónica), "familia ensamblada", "hijastros", "segundas nupcias", "celos" y "nido vacío" no tenían ninguna cobertura dedicada. Se priorizó **celos e inseguridad en el matrimonio** por ser un dolor de pareja extremadamente común y con base bíblica rica y matizada (a diferencia de temas como adicciones o enfermedad crónica, que requieren un tratamiento más largo y posiblemente remisión a ayuda profesional especializada desde el inicio). El hueco de intimidad física/sexual, señalado en rondas anteriores, sigue diferido por requerir un tratamiento de tono aún más cuidadoso.

| Keyword | Intención | URL asignada | Tipo de contenido | Etapa del embudo | CTA | Contenidos secundarios | Riesgo de canibalización |
|---|---|---|---|---|---|---|---|
| celos en el matrimonio / inseguridad en la pareja | Informacional / crisis de confianza | `/blog/celos-e-inseguridad-en-el-matrimonio` | Satélite | MOFU | Conocer próximos pasos | `como-recuperar-la-comunicacion-con-mi-pareja`, `5-senales-de-que-tu-matrimonio-necesita-sanidad-emocional` | Bajo — primer y único artículo del sitio sobre celos/inseguridad |

Contenido publicado: `celos-e-inseguridad-en-el-matrimonio.mdx`, categoría "Crisis matrimonial", ~926 palabras. Reencuadre central: distingue el celo bíblico protector/exclusivo (Cantares 8:6) del celo destructivo que nace de la inseguridad (Proverbios 6:34), y mide ambos contra el estándar de 1 Corintios 13:4 ("el amor no tiene envidia"). Estructura variada respecto a los dos artículos anteriores: sin listas numeradas (a diferencia del de suegros) y con una sola lista de viñetas breve en vez de dos secciones con viñetas (a diferencia del de cónyuge no creyente); FAQ de 4 preguntas en vez de 3. Cubre ambos lados de la dinámica: consejos para quien siente los celos y para quien los recibe sin motivo real, con nota de seguridad explícita cuando el control cruza a una situación de abuso. Versículos verificados por búsqueda contra RVR1960 antes de usarlos (confirmando específicamente la edición RVR1960 y no NVI/DHH, que tienen redacciones distintas): Cantares 8:6, Proverbios 6:34, 1 Corintios 13:4. Imagen: Pexels 8559951 (pareja sentada en un sofá, cada uno mirando hacia otro lado, distancia y preocupación silenciosa sin dramatismo), verificada visualmente — se descartó como alternativa Pexels 6669853 (banco de madera exterior, estética más "boho/lifestyle" que no calza con el tono del sitio) y se rechazó Pexels 6670203 (no correspondía al tema, era una sesión de fotos tipo compromiso).

## Nuevo hueco cubierto: familia ensamblada / conflictos con hijastros (2026-09-19)

Cadencia habitual: PR #80 ("suegros" y "celos") seguía siendo el único sin mergear — un solo PR, no calificaba como "varios acumulados" bajo la regla acordada, así que se continuó en la misma rama de trabajo de esta sesión.

Se revisó de nuevo la lista de temas clásicos sin cobertura detectada en la ronda anterior (`grep` sobre los ~64 artículos publicados): "pornografía", "alcohol", "enfermedad" (crónica), "hijastros"/"familia ensamblada", "segundas nupcias" y "nido vacío" seguían sin ninguna mención. Se priorizó **familia ensamblada / conflictos con hijastros** sobre pornografía, alcohol o enfermedad crónica porque estos últimos requieren desde el inicio una remisión más fuerte a ayuda profesional especializada (adicciones, salud mental) y un tratamiento editorial más largo — mientras que el tema de hijastros es un dolor de pareja extremadamente común en matrimonios de segunda vez, con base bíblica clara y aplicable sin necesitar ese nivel de derivación clínica. El hueco de intimidad física/sexual, señalado en rondas anteriores, sigue diferido.

| Keyword | Intención | URL asignada | Tipo de contenido | Etapa del embudo | CTA | Contenidos secundarios | Riesgo de canibalización |
|---|---|---|---|---|---|---|---|
| familia ensamblada / conflictos con hijastros en el matrimonio | Informacional / crisis de comunicación | `/blog/familia-ensamblada-conflictos-con-hijastros-en-el-matrimonio` | Satélite | MOFU | Guía gratuita | `como-hablar-con-mi-esposo-sin-pelear`, `como-recuperar-la-comunicacion-con-mi-pareja` | Bajo — primer y único artículo del sitio sobre familias ensambladas/hijastros |

Contenido publicado: `familia-ensamblada-conflictos-con-hijastros-en-el-matrimonio.mdx`, categoría "Comunicación en pareja", ~1038 palabras. Reencuadre central: la disciplina y el lugar de los hijastros como el punto de fricción más común en una familia ensamblada, con un acuerdo práctico (el padre/madre biológico lidera la disciplina mientras el vínculo del padrastro/madrastra se construye) presentado en prosa, sin listas ni numeración — variación deliberada respecto a los tres artículos anteriores, que sí usaban listas o numeración. FAQ de 5 preguntas (frente a 3 y 4 de las rondas previas). Versículos verificados por búsqueda contra RVR1960 antes de usarlos: Rut 1:16 (lealtad elegida, no de sangre), Efesios 6:4 y Colosenses 3:21 (mismo estándar de crianza para ambos cónyuges, sin importar el vínculo biológico). Imagen: Pexels 8550724 (padre/padrastro y adolescente en una conversación seria en una escalera, sin dramatismo ni sonrisas forzadas de foto de stock), verificada visualmente.
