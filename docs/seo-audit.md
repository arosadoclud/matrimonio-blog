...
Esto ya cumple, en sustancia, con lo pedido en la Fase 10. **No se creó un componente visual reutilizable de advertencia de seguridad** en esta pasada (para no arriesgar el diseño existente sin una revisión visual en navegador) — queda documentado como mejora recomendada en la sección "Decisiones pendientes de aprobación" más abajo.

## 7. Decisiones que requieren aprobación manual

1. **Componente visual reutilizable de "advertencia de seguridad"** (Fase 10): hoy el tratamiento es texto en prosa dentro de cada artículo (ya responsable), no un componente UI destacado (ej. caja con ícono/color distintivo). Se recomienda crear `components/SafetyNotice.tsx` e insertarlo en los ~12 artículos que ya tocan el tema, pero requiere decisión de diseño (color, ubicación, copy exacto) antes de tocar la UI en producción.
2. **Redacción de `/indexacion`**: menciona `restauratumatrimonio.org` (el landing de ventas) en vez de `restauratumatrimonio-blog.com` (este blog) al hablar de Search Console/sitemap. Es una página interna (`noindex,nofollow`) así que no afecta SEO externo, pero el contenido es confuso para quien la use como guía operativa. Se documentó la corrección sugerida en `docs/manual-seo-setup.md` en vez de reescribir la página, para no asumir qué versión final se quiere mantener.
3. **Categorías con solo 1-2 artículos** ("Comunicación en pareja": 2, "Testimonios": 2): no se marcaron `noindex` porque sí tienen contenido real y publicado, pero son candidatas a "ampliar" según el plan editorial de 90 días antes de invertir más en enlazado interno hacia ellas.
4. **Textos legales** (`/privacidad`, `/terminos`, `/cookies`, `/afiliados`): son razonables como base, pero **no sustituyen asesoría legal profesional** — se recomienda revisión por un abogado antes de depender de ellos en un contexto de captación de datos y venta de un programa pagado.
5. **Dominio canónico confirmado por el propietario en esta sesión:** `restauratumatrimonio-blog.com` sin `www` (ver sección 2). Si esto cambia en el futuro, solo hace falta actualizar `siteConfig.domain`/`url` en `lib/site.ts` — el resto del código ya lo consume desde ahí.
