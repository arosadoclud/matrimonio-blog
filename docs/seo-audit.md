...
5. **Consentimiento de cookies:** el mecanismo técnico ya está implementado (`lib/consent.ts`, `NEXT_PUBLIC_REQUIRE_ANALYTICS_CONSENT`, ver `docs/analytics-tracking.md` y `docs/manual-seo-setup.md` sección 3b) — sigue pendiente **solo** la decisión legal/de negocio de activarlo en producción y aprobar el texto final del banner (marcado `TODO(legal)` en el código).
6. **Enriquecer `CtaClick`/`trackHotmartCtaClick`** con `article_slug`/`article_category`/`cta_location`/`content_cluster`/`destination_url`/`cta_text` — **implementado** (ver `docs/analytics-tracking.md`).
7. **Microsoft Clarity** — implementación técnica lista (`NEXT_PUBLIC_CLARITY_PROJECT_ID`), pendiente solo que el propietario cree el proyecto real y provea el ID (ver `docs/manual-seo-setup.md` sección 4).
8. **Diferenciación de artículos con riesgo de canibalización** (clústeres "versículos" y "señales de restauración") — propuesta editorial completa en `docs/content-audit.md` y `docs/keyword-map.md`; ninguna fusión ejecutada, decisión final pendiente de datos de Search Console.
9. **Categorías "Comunicación en pareja" y "Testimonios"** — se mantienen indexables (justificación en `docs/content-audit.md`), se les agregó introducción propia y 5 ideas de contenido cada una; ninguna se marcó `noindex` ni se movió de URL.
